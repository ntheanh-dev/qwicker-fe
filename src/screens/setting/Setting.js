        ...
        <View className="flex-col flex-1 space-y-3 pt-3">
            ...
            >
                <View className="h-full w-full px-4 pt-5 flex-col">
                    <Text className="text-center text-xl font-semibold mb-3">Thay đổi mật khẩu</Text>
                    <View className="rounded-lg border-2 border-[#D1D1D1] p-4 bg-[#FFFFFF] mb-4">
                        <TextInput
                            value={data.oldPassword}
                            placeholderTextColor={'#A5A5A5'} placeholder="Mật khẩu cũ"
                            onChangeText={txt => updateData({ oldPassword: txt })}
                            secureTextEntry={!data.showOldPassword}
                            autoFocus={true}
                        />
                        {data.showOldPassword ? <TouchableOpacity onPress={() => updateData({ showOldPassword: false })} className="absolute right-3 top-0 translate-y-4">
                            <Entypo name="eye" size={20} color="#A5A5A5" />
                        </TouchableOpacity> :
                            <TouchableOpacity onPress={() => updateData({ showOldPassword: true })} className="absolute right-3 top-0 translate-y-4">
                    ...
                    <TouchableOpacity
                        ...
                        className={`rounded-lg w-full flex justify-center items-center h-14 ${fullFill() ? 'bg-[#3422F1]' : 'bg-gray-400'}`}
                    >
