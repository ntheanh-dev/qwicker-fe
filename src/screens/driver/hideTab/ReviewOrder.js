            ...
            {index === 1 ? (
                ...
                <View className="absolute left-0 right-0 bottom-0 bg-white border-t border-gray-300 px-4 py-6">
                    <TouchableOpacity
                        ...
                        underlayColor={'rbga(0,0,0,0)'}
                        className={`rounded-lg w-full flex justify-center items-center h-14 ${isReviewed ? 'bg-gray-600' : 'bg-[#3422F1]'}`}
                    >
                        <Text className="text-lg font-medium text-white" >{isReviewed ? 'Đã đánh giá' : 'Đánh giá ngay'}</Text>
                    </TouchableOpacity>
                </View>
            )}
